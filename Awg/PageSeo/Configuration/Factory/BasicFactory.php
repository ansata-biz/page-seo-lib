...
class BasicFactory implements FactoryInterface
{
   ...
   */
  function __construct($provider, $route_configuration_class = '\Awg\PageSeo\Configuration\RouteConfiguration')
  {
  ...
  public function getRouteConfiguration($routeName)
  {
    if (!isset($this->cache[$routeName]))
    {
      ...
      $this->cache[$routeName] = new $this->route_configuration_class(
        @$config['text'], @$config['title'], @$config['description'], @$config['keywords']
      );
    }
